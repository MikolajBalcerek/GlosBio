...
class Recorder extends Component {
	...
	saveAll() {
		if (this.state.blob_audio_data.length > 0) {
			...
			this.handleClickVariant(
				"Nie można zapisać pliku, nie został nagrany!",
				"error"
			);
	}
	onInputChange(e) {
	...
	onPressButtonUpload(value, onlyOne, counter) {
		if (this.state.recorded && this.state.username) {
			...
			fd.append(
				"file",
				this.state.blob_audio_data[value].blob
					? this.state.blob_audio_data[value].blob
					: this.state.blob_audio_data[value]
			);
			...
			return axios
				...
				.then(function(response) {
					self.handleClickVariant(
						`Plik ${value} zapisano poprawnie! ${
							response.data.text
						} `,
						"success"
					);
					...
					!onlyOne &&
						value < self.state.blob_audio_data.length &&
						self.onPressButtonUpload(value, false, counter + 1);
				...
				.catch(function(error) {
					...
					self.handleClickVariant(
						`Podczas zapisu pliku ${value} wystąpił błąd!`,
						"error"
					);
					console.log(error);
				});
			...
			if (!this.state.recorded) {
				return this.handleClickVariant(
					"Nie można zapisać pliku, nie został nagrany!",
					"error"
				);
			}
			if (!this.state.username) {
				return this.handleClickVariant(
					"Nie można zapisać pliku, podaj imię i nazwisko!",
					"error"
				);
			}
		}
					...
					>
						...
						>
							...
							<Typography variant="subheading" gutterBottom>
								{this.state.recognizedText !== "None" &&
							...
							>
								...
								>
									<Button color="primary" variant="contained">
										Upuść tutaj plik/i, lub kliknij aby
										wybrać plik/i z komputera
									</Button>
								</Files>
							</div>
								...
								{this.state.recorded && (
									<div style={{ width: "100%" }}>
										...
										>
											...
											>
												{this.state.blob_audio_data.map(
													(sound, i) =>
														_.find(
															this.state.uploaded,
															{ id: i }
